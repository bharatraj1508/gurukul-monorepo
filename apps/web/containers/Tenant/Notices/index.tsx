...
export default function TenantNoticesContainer() {
  ...
  const canCreateAny =
    canCreateClass || canCreateTeachers || canCreateSchoolWide;

  const tabs = [{ id: 'ALL', label: 'All Notices', show: true }];
  if (canCreateClass || hasPermission(PERMS.notice.viewAll))
    tabs.push({ id: 'CLASS', label: 'Class', show: true });
  if (canCreateTeachers || hasPermission(PERMS.notice.viewAll))
    tabs.push({ id: 'TEACHERS_ONLY', label: 'Teachers Only', show: true });
  if (canCreateSchoolWide || hasPermission(PERMS.notice.viewAll))
    tabs.push({ id: 'SCHOOL_WIDE', label: 'School Wide', show: true });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {notices.map((notice) => {
              ...
              const statusColors = {
                ...
                EXPIRED:
                  'text-zinc-500 bg-zinc-100 dark:bg-zinc-800/50 dark:text-zinc-400',
              };
              ...
              const isOwner =
                !!notice.createdBy && notice.createdBy === profile?.id;
              const canEditNotice =
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex flex-wrap gap-2">
                        <Badge
                          variant="outline"
                          className={cn(
                            'border-none text-[10px] font-semibold tracking-wider',
                            scopeColors[notice.scope],
                          )}
                        >
                        ...
                        <DropdownMenu>
                          ...
                          >
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-8 w-8 -mr-2 -mt-2 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity"
                            >
                    ...
                    {notice.scope === 'CLASS' && notice.classes?.length > 0 && (
                      <div className="text-xs text-zinc-500 dark:text-zinc-400 mb-3 line-clamp-1">
                        Target:{' '}
                        {notice.classes.map((c) => c.class.name).join(', ')}
                      </div>
                    )}
                    ...
                    <div className="mt-auto pt-4 flex flex-col gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>
                          {format(new Date(notice.startDate), 'MMM d')} -{' '}
                          {format(new Date(notice.endDate), 'MMM d, yyyy')}
                        </span>
                      </div>
                    </div>
                  </div>
